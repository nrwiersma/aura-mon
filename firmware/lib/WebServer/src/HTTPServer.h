...
public:
    ...
    using HookFunction = std::function<ClientFuture(const String& method, const String& url, const String& params, WiFiClient* client, ContentTypeFunction contentType)>;
    void addHook(HookFunction hook) {
        if (_hook) {
            auto previousHook = _hook;
            _hook = [previousHook, hook](const String & method, const String & url, const String& params, WiFiClient * client, ContentTypeFunction contentType) {
                auto whatNow = previousHook(method, url, params, client, contentType);
                if (whatNow == CLIENT_REQUEST_CAN_CONTINUE) {
                    return hook(method, url, params, client, contentType);
                }
                return whatNow;
            };
        } else {
