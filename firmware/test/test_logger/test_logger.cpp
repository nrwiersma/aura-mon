...
void test_format_substitution_in_serial() {
    testLogger->infof("val=%d name=%s", 42, "thing");
...
void test_restart_marker_written_only_once() {
    testLogger->infof("msg one");
    testLogger->infof("msg two");
...
void test_infof_long_message() {
    ...
    testLogger->infof("%s%s%s", repeated, repeated, repeated); // 108 chars
...
void test_errorf_long_message() {
    const char *repeated = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; // 36 chars
    testLogger->errorf("%s%s%s", repeated, repeated, repeated); // 108 chars
...
void test_mixed_levels_only_info_and_error_in_file() {
    testLogger->debugf("debug line");
    testLogger->infof("info line");
    testLogger->errorf("error line");
...
void test_multiple_serial_writes_accumulate() {
    testLogger->infof("first");
    testLogger->infof("second");
    testLogger->infof("third");
