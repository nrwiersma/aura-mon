...
void test_sync_info_creates_device_for_new_info() {
    ...
    TEST_ASSERT_EQUAL_STRING("Sensor A", devices[0]->name);
}
...
void test_sync_info_updates_existing_device() {
    ...
    auto *info    = new inputDeviceInfo(1);
    info->enabled = true;
    info->name    = strdup("Old");
    deviceInfos[0] = info;
    syncDeviceInfo();

    // Update the info.
    free(const_cast<char *>(deviceInfos[0]->name));
    deviceInfos[0]->name    = strdup("New");
    deviceInfos[0]->enabled = false;

    syncDeviceInfo();

    TEST_ASSERT_EQUAL_STRING("New", devices[0]->name);
...
void test_sync_info_null_info_name_gives_null_device_name() {
    ...
    TEST_ASSERT_NULL(devices[2]->name);
}
...
void test_sync_data_creates_data_entry_for_active_device() {
    ...
    devices[0]->name          = strdup("Dev1");

    syncDeviceData();

    TEST_ASSERT_NOT_NULL(deviceData[0]);
    TEST_ASSERT_EQUAL_STRING("Dev1", deviceData[0]->name);
...
void test_sync_data_null_device_name_gives_null_data_name() {
    ...
    TEST_ASSERT_NULL(deviceData[0]->name);
}
